...
string file_1 = "../include/left.png";  // first image
string file_2 = "../include/right.png";  // second image
string file_3 = "../include/disparity.png";
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    error = GetPixelValue(img1, kp.pt.x+x, kp.pt.y+y)-GetPixelValue(img2, kp.pt.x+x+dx, kp.pt.y+y+dx);
                    if (inverse == false) {
                        // Forward Jacobian
                        J = -1.0*Eigen::Vector2d(0.5*(GetPixelValue(img2, kp.pt.x+dx+x+1, kp.pt.y+dy+y)-GetPixelValue(img2, kp.pt.x+dx+x-1, kp.pt.y+dy+y)),
                                                 0.5*(GetPixelValue(img2, kp.pt.x+dx+x, kp.pt.y+dy+y+1)-GetPixelValue(img2, kp.pt.x+dx+x, kp.pt.y+dy+y-1)));
                    } else {
                        // Inverse Jacobian
                        J = -1.0*Eigen::Vector2d(0.5*(GetPixelValue(img1, kp.pt.x+x+1, kp.pt.y+y)-GetPixelValue(img1, kp.pt.x+x-1, kp.pt.y+y)),
