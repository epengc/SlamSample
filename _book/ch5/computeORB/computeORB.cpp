...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        u = kp.pt.x;
        v = kp.pt.y;
        if(u<half_patch_size || v<half_patch_size || u>cols+1-half_patch_size || v>rows+1-half_patch_size) continue;
        kp.angle = 0;
        float m10=0;
        float m01=0;
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            cv::Point2f ORB_pattern_p(_cos*ORB_pattern[4*i]-_sin*ORB_pattern[4*i+1], 
                                    _sin*ORB_pattern[4*i]+_cos*ORB_pattern[4*i+1]);
            cv::Point2f ORB_pattern_q( _cos*ORB_pattern[4*i+2]-_sin*ORB_pattern[4*i+3],
    ...
    for(size_t i=0; i<desc1.size(); i++){
        ...
        for(size_t j=0; j<desc2.size(); j++){
            ...
            for(size_t k=0; k<256; k++){
                d_hamming += desc1[i][k]^desc2[j][k];
            }
            if(d_hamming<d_min && d_hamming<d_max){
                d_min = d_hamming;
                index_desc2 = j;
            }
        }
        if(d_min < d_max){
            matches.push_back(cv::DMatch(i, index_desc2, d_min));
        }
    } 
    for (auto &m: matches) {
